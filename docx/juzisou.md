...
## 简介
`wenansou`是一个用户友好的Python库，它允许您轻松地向网络服务发送请求，并获取响应。这个库特别适合那些需要从外部服务获取数据的应用程序。
...
## 安装步骤
在开始使用`wenansou`之前，您需要确保已经安装了所有必要的依赖库。请按照以下步骤操作：
...
2. 输入以下命令并按回车：
   ...
   这个命令会安装`wenansou`所需的所有库。

## 使用方法

### 步骤 1: 导入API
在您的Python代码中，您需要先导入`wenansou`：

```python
from MZAPI.wenansou import WenAnSou
```

这行代码告诉Python去哪里找到`wenansou`函数。
